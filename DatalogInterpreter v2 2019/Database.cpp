...
Database::Database(DataLog dataLog, ostream * out) {
	...
	*out << "Rule Evaluation" << endl;
	...
	for (i = 0; !stable; i++)
		evaluateRules();
	*out << endl;
	*out << "Schemes populated after " << i << " passes through the Rules." << endl;
	*out << endl;
	//printRelations();
...
void Database::evaluateRules() {
	...
	for (Rule rule : rules) {
		*out << rule.toString();
		Scheme conclusion = toScheme(rule.getHeadPredicate());
		vector<Scheme> premises = toScheme(rule.getPredicateList());
...
vector<Scheme> Database::toScheme(vector<Predicate> predicates) {
	...
	for (Predicate predicate : predicates) {
		schemes.push_back(toScheme(predicate));
	}
	return schemes;
}
...
vector<Tuple> Database::toTuple(vector<Predicate> predicates) {
	vector<Tuple> tuples;
	for (Predicate predicate : predicates) {
		tuples.push_back(toTuple(predicate));
	}
	return tuples;
}
